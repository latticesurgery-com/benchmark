...
from app_oriented_benchmarks_adapted import _set_up_path
...
try:
    ...
    try:
        ...
        precalculated_data = json.load(open(
            'external/QC-App-Oriented-Benchmarks/hamiltonian-simulation/_common/precalculated_data.json',
            'r'))
...
def HamiltonianSimulation(n_spins, K, t, w, h_x, h_z):
    ...
    with open("HamiltonianSimulationParams.txt", "a") as f:
        f.write(f"n_spins={n_spins}, K={K}, t={t}, w={w}, h_x={h_x}, h_z={h_z}\n")
    # allocate qubits
...
def xxyyzz_opt_gate(tau):
    ...
    qc.rz(np.pi / 2, qr[1])
    qc.cx(qr[1], qr[0])
    qc.rz(np.pi * gamma - np.pi / 2, qr[0])
    qc.ry(np.pi / 2 - np.pi * alpha, qr[1])
    qc.cx(qr[0], qr[1])
    qc.ry(np.pi * beta - np.pi / 2, qr[1])
    qc.cx(qr[1], qr[0])
    qc.rz(-np.pi / 2, qr[0])
