...
if __name__ == "__main__":
    ...
    for qubit_num in [5,6,7, 8, 9, 10]:
        start = time.time()
        command_output = subprocess.check_output(
            f" {PYTHON_EXECUTABLE} {GENERATOR_EXECUTABLE} {qubit_num}", shell=True).decode('utf-8')
        command_output = command_output[:-1] # remove trailing newline
        print(f"({qubit_num}, {command_output}, {time.time() - start}),")

        #  | {SLICER_EXECUTABLE} -o -f progress -l grover/grover_l.txt